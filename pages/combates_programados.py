...
if len(st.session_state.combates_programados) > 0:
    ...
    with col1:
        ...
        st.session_state.lista_patrocinadores = pd.Series(
    ...
    with col2:
        ...
        if st.session_state.seleccion != []: 
            ...
            st.dataframe(                                   # ~ Listado con fecha, arena, modo, control, c/e ~ #
                data = [evento],
            ...
            with col1:                                          # ~ Listado de Equipos(Robots) ~ #
                ...
                st.dataframe(                                   
                    pd.Series(
                    data = evento["Equipo_A"].keys(), 
                    name = "Equipo A" if evento["Modo"] == "Equipo vs Equipo" else "Combatiente A"),
                    hide_index = True
                    )
                
                st.dataframe(
                    pd.Series(
                    data = evento["Equipo_B"].keys(), 
                    name = "Equipo B" if evento["Modo"] == "Equipo vs Equipo" else "Combatiente B"),
                    hide_index = True
                    )
            ...
            with col2:                                          # ~ Listado de Equipos(Armas) ~ #
                ...
                st.dataframe(
                    pd.Series(
                    data = evento["Equipo_A"].values(), 
                    name = "Armas Equipadas Equipo A" if evento["Modo"] == "Equipo vs Equipo" else "Armas Equipadas Combatiente A"),
                    hide_index = True
                    )
                
                st.dataframe(
                    pd.Series(
                    data = evento["Equipo_B"].values(), 
                    name = "Armas Equipadas Equipo B" if evento["Modo"] == "Equipo vs Equipo" else "Armas Equipadas Combatiente B"),
                    hide_index = True
                   )
    ...
    with col2:
        ...
        with st.form(
            ...
            with st.container(
                ...
                Eliminar = st.form_submit_button(                       # ~ Boton eliminar ~ #
                    label = "**:red[Eliminar]**",
                    help = "Botón para eliminar un evento programado.")          
